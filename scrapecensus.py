...
def scrapeCensusGeoData(site):
    ...
    for result in results:
        ...
        if url.startswith('/Areas/Census Tract/District of Columbia/District of Columbia/Census%20Tract'):
            ...
            # Scrape latitude
            searchResult = soup.find('td', string='Latitude of the Internal Point')
            lat = searchResult.next_sibling.get_text()

            # Scrape longitude
            searchResult = soup.find('td', string='Longtitude of the Internal Point')
            lon = searchResult.next_sibling.get_text()

            censusGeoData.append({'Census Tract': name, 'Latitude': lat, 'Longitude': lon})
...
def createCensusGeoTable():
    ...
    for lab,row in df.iterrows():
        stmt = insert(censusTractCoordinates).values(census=row['Census Tract'], latitude=row['Latitude'], longitude=row['Longitude'])
        result = connection.execute(stmt)

if __name__ == '__main__':
    censusGeoData = []

    scrapeCensusGeoData('http://www.usboundary.com/Areas/Census%20Tract/District%20of%20Columbia/1')
    scrapeCensusGeoData('http://www.usboundary.com/Areas/Census%20Tract/District%20of%20Columbia/2')
    scrapeCensusGeoData('http://www.usboundary.com/Areas/Census%20Tract/District%20of%20Columbia/3')
